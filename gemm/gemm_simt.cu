...
// thread tiling
template<int BM, int BN, int BK, int TM, int TN, int BLOCK_SIZE>
__global__ void gemm_kernel_block_thread_tile_double_buffer(
    const float* __restrict__ A, // m x k row-major
    const float* __restrict__ B, // k x n row-major
    float * __restrict__ C, // m x n row-major
    int m, int n, int k
    ...
    // load a cooperatate with all thread within current block.
    auto load_a_to_smem = [&](int k_iter_idx, int buffer_idx)
    {
        for (int i = tid; i < BM * BK; i += BLOCK_SIZE)
        {
            int row = i / BK;
            int col = i % BK;
            As[buffer_idx][row][col] = (m_offset + row < m && k_iter_idx * BK + col < k) ? *(A_ptr + row * k + k_iter_idx * BK + col) : 0;
        }
    };

    auto load_b_to_smem = [&](int k_iter_idx, int buffer_idx)
    {
        for (int i = tid; i < BK * BN; i += BLOCK_SIZE)
        {
            int row = i / BN;
            int col = i % BN;
            Bs[buffer_idx][row][col] = (k_iter_idx * BK + row < k && n_offset + col < n) ? *(B_ptr + (k_iter_idx * BK + row) * n + col) : 0;
        }
    };
    ...
    auto compute = [&](int buffer_idx){
        for(int k_smem_idx = 0; k_smem_idx < BK; k_smem_idx++){
            // load a from smem to regs
            for(int i = 0; i < TM; i++){
                Ar[i] = As[buffer_idx][i + row_th_idx * TM][k_smem_idx];
            }
            // load b from smem to regs
            for(int i = 0; i < TN; i++){
                Br[i] = Bs[buffer_idx][k_smem_idx][col_th_idx * TN + i];
            }
            for(int i = 0; i < TM; i++){
                for(int j = 0; j < TN; j++){
                    Cr[i][j] += Ar[i] * Br[j]; 
    ...
    #pragma unroll
    for(int k_iter_idx = 0; k_iter_idx < k_iter; k_iter_idx++){
        int cur_buffer_idx = k_iter_idx % 2;
        int next_buffer_idx = (k_iter_idx + 1) % 2;

        __syncthreads();

        load_a_to_smem(k_iter_idx + 1, next_buffer_idx);
        load_b_to_smem(k_iter_idx + 1, next_buffer_idx);
...
void gemm_block_thread_tile_double_buffer(
    const float* __restrict__ A,
    const float* __restrict__ B,
    float * __restrict__ C,
    int m, int n, int k
    ...
    dim3 block(BLOCK_SIZE, 1, 1);
    dim3 grid((m + BM - 1) / BM, (n + BN - 1) / BN);
    gemm_kernel_block_thread_tile_double_buffer<BM, BN, BK, TM, TN, BLOCK_SIZE><<<grid, block>>>(A, B, C, m, n, k);
}
